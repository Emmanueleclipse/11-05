...
import { TouchableOpacity, Dimensions, View, Image, ScrollView, FlatList, SectionList } from 'react-native';
...
class ShopList extends React.Component {
        ...
        return (
            ...
            <List>
                ...
                {/* <SectionList
                    ...
                    renderItem={({ item }) => <Item title={item.ingredient} />}
                    renderSectionHeader={({ section: { recipeName } }) => (
                        <Text style={styles.header}>{item.recipeName}</Text>
                    )}
                /> */}
    ...
    async fetchRecipes() {
        ...
        // const results = recipesFav.reduce((recipes, { recipeId, recipeName, ingredient, uniqueId }) => {

        //     (recipes[recipeId] = recipes[recipeId] || []).push({ recipeName: recipeName, recipeId: recipeId, ingredient: ingredient, uniqueId: uniqueId });
        //     return recipes;
        // }, {});

        const results = recipesFav.reduce(function (recipeId, recipeName, ingredient, uniqueId) {
            return `{${recipeName}: {recipeId: ${recipeId}, recipeName: ${recipeName}, ingredient: ${ingredient}, uniqueId: ${uniqueId} }}}`
        })
        console.log(results)
        // //console.log(results)
        // const arrayOfObj = Object.keys(results).map(key => results[key])

        // const recipesArray = arrayOfObj || [];
        // var testArray = [[{name: test, you:no}],[{name: test, you: yes}, {name: test2, you: no}]]
        // this.setState({
        //     ...this.state,
        //     shopping_list: recipesArray
        // });
